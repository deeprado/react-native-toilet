...
export default class PPRefreshLayout extends Component {
  ...
  constructor(props) {
    ...
    this.state = {
      dataList: [],
      refreshing: false,
      loadMoreFlag: Constant.load_more_flag.LOADING_COMPLETE,
    };
  }
  ...
  _renderFooter = () => {
    ...
    switch (loadMoreFlag) {
      case Constant.load_more_flag.LOADING:
        return (
      ...
      case Constant.load_more_flag.LOADING_COMPLETE:
        return null;
      case Constant.load_more_flag.LOADING_COMPLETE_NO_DATA:
        return (
          <View style={styles.footerContainer}>
            <Text style={{color: '#333', fontSize: 14}}>没有更多数据</Text>
          </View>
        );
      case Constant.load_more_flag.LOADING_FAIL:
        return (
  ...
  onLoadMore = () => {
    const loadMoreFlag = this.state.loadMoreFlag;
    if (
      loadMoreFlag === Constant.load_more_flag.LOADING ||
      loadMoreFlag === Constant.load_more_flag.LOADING_FAIL ||
      loadMoreFlag === Constant.load_more_flag.LOADING_COMPLETE_NO_DATA
    ) {
      return;
    }
    this.setState({loadMoreFlag: Constant.load_more_flag.LOADING});
  ...
  scrollToTop = (animated = true) => {
    this.getFlatList() &&
      this.getFlatList().scrollToIndex({
        viewPosition: 0,
        index: 0,
        animated: animated,
      });
   ...
   */
  isLoadMore() {
    return this.state.loadMoreFlag === Constant.load_more_flag.LOADING;
  }
...
const styles = StyleSheet.create({
  footerContainer: {
    ...
    flexDirection: 'row',
    width: Constant.common_param.SCREEN_WIDTH,
    height: 40,
