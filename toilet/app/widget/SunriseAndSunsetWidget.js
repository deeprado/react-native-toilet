...
export default class SunriseAndSunsetWidget extends Component {
  ...
  constructor(props) {
    ...
    percent = parseFloat(
      (moment().get() - sunriseMillis) / (sunsetMillis - sunriseMillis),
    );
  ...
  render() {
    ...
    const sunCircleX =
      sunriseAndSunsetWidth * percent + leftAndRightPadding - sunRadius;

    let diffSunCircleXAndSunriseAndSunsetWidth =
      sunCircleX -
      leftAndRightPadding +
      sunRadius -
      0.5 * sunriseAndSunsetWidth;
    let sunCircleY, circleAngle;
    if (diffSunCircleXAndSunriseAndSunsetWidth !== 0) {
      if (diffSunCircleXAndSunriseAndSunsetWidth > 0) {
        // 太阳圆圈在右边
        circleAngle = Math.acos(
          diffSunCircleXAndSunriseAndSunsetWidth /
            (0.5 * sunriseAndSunsetWidth),
        );
      ...
      diffSunCircleXAndSunriseAndSunsetWidth = Math.max(
        diffSunCircleXAndSunriseAndSunsetWidth,
      );
      sunCircleY = Math.sqrt(
        Math.pow(0.5 * sunriseAndSunsetWidth, 2) -
          Math.pow(diffSunCircleXAndSunriseAndSunsetWidth, 2),
      );
    ...
    const swipePath = this.makeArcPath(
      leftAndRightPadding,
      leftAndRightPadding,
      -CIRCLE * 0.25,
      endAngle,
    ...
    return (
      <View
        ...
        <Surface width={SCREEN_WIDTH} height={height}>
          <Group>
            <Shape
              d={path}
              stroke={'#fff'}
