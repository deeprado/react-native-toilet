...
export default class HourWeatherComponent extends Component {
  ...
  convert = (item, index) => {
    ...
    let WeatherLineBean;
    if (index === 0) {
      WeatherLineBean = this.generateWeatherLineBean(
        currentTemp,
        -1,
        hourfc[1].wthr,
      );
    ...
    return (
      <View
        ...
        <Text
          ...
          {WeatherTimeUtil.getWeatherTime(item.time)}
        </Text>
        <Image
          source={WeatherIconUtil.getWeatherIconByType(item.type)}
          style={{
            width: 24,
            height: 24,
            resizeMode: 'cover',
        ...
        <WeatherLineWidget WeatherLineBean={WeatherLineBean} />
      </View>
    );
  };
