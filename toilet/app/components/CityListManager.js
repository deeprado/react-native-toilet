...
export default class CityListManager {
  ...
  static addCity(
    ...
    const cityItem = this.generateCityItem(
      cityId,
      cityName,
      district,
      province,
    ...
    this.findCity(cityId, (hasCity, cityList) => {
      if (cityList && cityList.length >= Constant.max_weather_city_number) {
        // 已经满10个城市
        ToastUtil.show(Constant.add_city_fill);
        return;
      }
      if (hasCity) {
        // 有这个城市
        ToastUtil.show(Constant.add_city_error);
      } else {
        // 没有这个城市
        if (!cityList) cityList = [];
        cityList.push(cityItem);
        AsyncStorage.setItem(Constant.city_list, JSON.stringify(cityList), () =>
          complete(cityList),
        );
      }
    });
  ...
  static updateCity(
    ...
    let updateCity = this.generateCityItem(
      cityId,
      cityName,
      district,
      province,
  ...
  static getCityList(callBack) {
    AsyncStorage.getItem(Constant.city_list, (error, result) => {
      let cityList = [];
      if (result) {
  ...
  static deleteCityList(deleteCityList = [], complete) {
    this.getCityList(cityList => {
      if (cityList && cityList.length > 0) {
        ...
        AsyncStorage.setItem(Constant.city_list, JSON.stringify(dataList), () =>
          complete(dataList),
        );
      }
    });
