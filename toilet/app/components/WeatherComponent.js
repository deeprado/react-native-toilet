...
import WeatherDataUtil from '../utils/WeatherDataUtil';
import Util from '../utils/Index';

import CityListManager from './CityListManager';
...
const APP_BAR_HEIGHT = Util.getAppBarHeight();
// let weatherDataBean;
...
export default class WeatherComponent extends Component {
    ...
    return (
      <View
        ...
        <WeatherHeaderWidget
          ...
          otherStyle={{position: 'absolute', top: 22}}
                ...
                callBack: (haveDelete, cityList) => {
                  if (haveDelete) {
                    this.props.refreshMainPage &&
                      this.props.refreshMainPage(cityList);
                  }
                },
              });
            }
  ...
  getWeatherData = () => {
    const url =
      'http://zhwnlapi.etouch.cn/Ecalender/api/v2/weather?' +
      'date=20170817&app_key=99817749&citykey=' +
      this.props.cityItem.cityId;
    fetch(url)
      .then(response => {
        if (response.ok) {
  ...
  onSuccess = weatherData => {
    ...
    CityListManager.updateCity(
      this.props.cityItem.cityId,
      this.props.cityItem.cityName,
      this.props.cityItem.district,
      this.props.cityItem.province,
      observe.temp,
      forecast15[1].high,
      forecast15[1].low,
      observe.wthr,
    ...
    for (let i = 0; i < ITEM_COUNT; i++) {
      dataList.push({
        itemType: i,
        weatherData: WeatherDataUtil.getWeatherData(i, weatherData),
      });
    }
